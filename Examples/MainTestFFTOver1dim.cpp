...
int main()
{
  ...
  cout << "Space spanned in dq3 (a.u.) WF3   "<< w1.dq3 << "\n";
  cout << "Space spanned in dx3 (a.u.) WF3   "<< w1.dx3 << "\n";
  ...
  // Start the gaussian
   ...
   for(int k=0;k<w1.n3;k++)
	...
	{ 	  
	  w1.w[w1.index(k,j,i)][0]= exp( -w1.x1[i]*w1.x1[i]-w1.x3[k]*w1.x3[k] );
   ...
   for(int k=0;k<w1.n3;k++)
	 ...
	 {
	   double norm=
	   factor1d*factor1d* w1.dx1*w1.dx2*w1.dx3*(w1.w[w1.index(k*skiper,j*skiper,i*skiper)][0]*w1.w[w1.index(k*skiper,j*skiper,i*skiper)][0]
			  +w1.w[w1.index(k*skiper,j*skiper,i*skiper)][1]*w1.w[w1.index(k*skiper,j*skiper,i*skiper)][1]);
	   fprintf(out6,"%e\n",norm);
	 }
