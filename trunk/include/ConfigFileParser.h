 ...
 * Changelog:
 *
 * extended for vector<> and adapted a bit for use with Parameter class.
 * (1.6.2006 Arvid Requate <arvid@fargonauten.de>)
 */
 /********************************************************************************/
...
public:
	...
	FileNotCreated(const string & f = "") : Error(f)
        {
	...
	KeyNotFound(const string & k = "") : Error(k) {}
    };
    /**
...
public:
  ...
  push_back(FromStringTo<T>(str.substr(last, _end-last)));
    ...
    throw ConfigFileParser::ConversionError("[" + s.str() + "]: " + e.what());
  }		
  }
};
