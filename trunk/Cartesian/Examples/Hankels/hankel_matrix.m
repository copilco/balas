function s_HT = hankel_matrix(p, R, N);
%HANKEL_MATRIX: Generates data to use for Hankel Transforms
%
%	s_HT = hankel_matrix(p, R, N)
%
...
%					=	J_p1(alpha)/R  or Jp1(alpha)/V where p1 = p+1
%
...
s_HT.r = s_HT.alpha * s_HT.R / s_HT.alpha_N1;
s_HT.v = s_HT.alpha / (2*pi * s_HT.R);
s_HT.kr = 2*pi * s_HT.v;
s_HT.V = s_HT.alpha_N1 / (2*pi * s_HT.R);
