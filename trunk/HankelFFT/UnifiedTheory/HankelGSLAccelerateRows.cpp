...
/*
	...
	for(int j=0;j<N;j++)
		for(int i=0;i<MM;i++)
		{
			F0mat.v[j*MM+i]=f0.v[j]/HH.m1.v[j];
			F1mat.v[j*MM+i]=f1.v[j]/HH.m1.v[j];
			
			//fprintf(sout0,"%d %e %e\n",j*MM+i,F0mat.v[j*MM+i],F1mat.v[j*MM+i]);
		}
	...
	for (int gg=0;gg<Nz;gg++)
	{
		...
		cblas_dgemm (CblasRowMajor, 
					 ...
					 1.0, HH.C.v, lda, F0mat.v, MM, 0.0, F20mat.v, MM);
		
		cblas_dgemm (CblasRowMajor, 
					 CblasNoTrans, CblasNoTrans, N, MM, N,
					 1.0, HH.C.v, lda, F1mat.v, MM, 0.0, F21mat.v, MM);
		...
		for(int j=0;j<N;j++)
			for(int i=0;i<N;i++)
			{
				F20.v[j]+=HH.C.v[j*MM+i]*F0.v[i];
				F21.v[j]+=HH.C.v[j*MM+i]*F1.v[i];
			}
		...
		for(int j=0;j<N;j++)
			for(int i=0;i<MM;i++)
			{
				double aux0r=F20mat.v[j*MM+i];
				double aux0i=F21mat.v[j*MM+i];
			
				phi.v[j]=(sqrt(K*K - 2*pi*HH.v.v[j]*2*pi*HH.v.v[j]) - K)*gg*dz; //The phase
				//	phi.v[i]=0.;//(sqrt(K*K - 2*pi*HH.v.v[i]*2*pi*HH.v.v[i]) - K)*gg*dz; //The phase
			
				F20mat.v[j*MM+i]=( aux0r*cos(phi.v[j])-aux0i*sin(phi.v[j]) );
				F21mat.v[j*MM+i]=( aux0i*cos(phi.v[j])+aux0r*sin(phi.v[j]) );
		...
		*/
		...
		if(gg==20)
		{
			...
			*/
			...
			for(int j=0;j<N;j++)
				for(int i=0;i<MM;i++)
					fprintf(sout0,"%d %e %e\n",j*MM+i,F20mat.v[j*MM+i],F21mat.v[j*MM+i]);
