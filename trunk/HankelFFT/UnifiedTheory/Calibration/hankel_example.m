...
%%Plots from the starting propagation point
figure
...
plot(a2(:,2),a2(:,3),'.r')
hold on
plot(EkrH_,'o','MarkerSize',10)
title('F2 vs EKrH_a')

%%Plots from the final propagation point Nz-1
figure
a4=importdata('../pout4.txt');
plot(a4(:,1),a4(:,2),'.r')
hold on
plot(ErHz(199*dz),'o','MarkerSize',10)
...
%%Plots from the final propagation point Nz-1
 ...
 plot(bb(:,1),bb(:,2),'.r')
 hold on
 plot(EkrH_ .* exp(i*phiz(199*dz)),'o','MarkerSize',10 )
 title('F2 vs EkrH_ .* exp(i*phiz(199*dz))')