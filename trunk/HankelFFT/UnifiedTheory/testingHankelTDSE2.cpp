...
int main()
{
	...
	double t_max = 25.;		//%	Maximum propagation distance
	int Nt = 400;//int(z_max/dz); 
	...
	for(int j=0;j<Nr;j++)
		for(int i=0;i<Nz;i++)
		{			
		  fretrieved.v[j*Nz+i][0]=1.;//exp(-z[i]*z[i]/9-HH.r.v[j][0]*HH.r.v[j][0]/9);///HH.m1.v[j][0];  // exp(-(HH.r.v[j][0]*HH.r.v[j][0]/w0/w0)-t[i]*t[i])/HH.m1.v[j][0];  //f.v[j][0]/HH.m1.v[j][0];
		  fretrieved.v[j*Nz+i][1]=0.;//exp(-z[i]*z[i]-HH.r.v[j][0]*HH.r.v[j][0])*f.v[j][1]/HH.m1.v[j][0];
		  V.v[j*Nz+i][0]=-6./sqrt(1.+(z[i]-5)*(z[i]-5)) -6./sqrt(1.+(z[i]+8)*(z[i]+8)+HH.r.v[j][0]*HH.r.v[j][0]);
		  // V.v[j*Nz+i][0]=-60./sqrt(1.+(z[i]-.8)*(z[i]-.8)+HH.r.v[j][0]*HH.r.v[j][0])
		  //  -60./sqrt(1.+(z[i]+.8)*(z[i]+.8)+HH.r.v[j][0]*HH.r.v[j][0]); 
		  V.v[j*Nz+i][1]=0.;
	...
	for(int j=0;j<Nr;j++)
		for(int i=0;i<Nz;i++)
		  norm+= fretrieved.v[j*Nz+i][0]* fretrieved.v[j*Nz+i][0]+ fretrieved.v[j*Nz+i][1]* fretrieved.v[j*Nz+i][1];
	
	for(int j=0;j<Nr;j++)
		for(int i=0;i<Nz;i++)
		  {
		    fretrieved.v[j*Nz+i][0]/=sqrt(norm);//* fretrieved.v[j*Nz+i][0]+ fretrieved.v[j*Nz+i][0]* fretrieved.v[j*Nz+i][0];
		    fretrieved.v[j*Nz+i][1]/=sqrt(norm);//* fretrieved.v[j*Nz+i][0]+ fretrieved.v[j*Nz+i][0]* fretrieved.v[j*Nz+i][0];
		  }
	...
	for (int gg=0;gg<Nt;gg++)
	{
		...
		for(int j=0;j<Nr;j++)
		  for(int i=0;i<Nz;i++)
		    {
		      F.v[j*Nz+i][0]=fretrieved.v[j*Nz+i][0]/HH.m1.v[j][0];  
		      F.v[j*Nz+i][1]=fretrieved.v[j*Nz+i][1]/HH.m1.v[j][0];
		    }
		...
		for(int j=0;j<Nr;j++)
			...
			{				
				...
				double psi=(- 2*pi*HH.v.v[j][0]*2*pi*HH.v.v[j][0]-w[i]*w[i])*dz;

				//complex phase=exp(-I*psi);
				
				F2.v[j*Nz+i][0]=aux0r*exp(psi)/Nz;//( aux0r*cos(psi)-aux0i*sin(psi) )/Nz;
				F2.v[j*Nz+i][1]=aux0i*exp(psi)/Nz;//0.;//( aux0i*cos(psi)+aux0r*sin(psi) )/Nz;
			}
		...
		for(int j=0;j<Nr;j++)
			...
			{				
				...
				double psi=(V.v[j*Nz+i][0])*dz;
				
				fretrieved.v[j*Nz+i][0]=aux0r*exp(-psi)/Nz;//( aux0r*cos(psi)-aux0i*sin(psi) )/Nz;
				fretrieved.v[j*Nz+i][1]=aux0i*exp(-psi)/Nz;//0.;//( aux0i*cos(psi)+aux0r*sin(psi) )/Nz;
			}
