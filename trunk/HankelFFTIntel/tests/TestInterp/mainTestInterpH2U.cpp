...
int main()
{
    cout << "\n\n\n/////////////////////////////////////////" << endl;
    cout << "/////////////////////////////////////////" << endl;
    cout << "///mainTestInterpH2U. Running example..." << endl;
    cout << "/////////////////////////////////////////" << endl;
	cout << "/////////////////////////////////////////" << endl;
	...
	fstream out2("outErrorH2U.txt",ios::out);
	...
	for(int j=0; j<Nr; j++)
		for (int i=0; i<Nz; i++)
        {
			wH.phi[wH.index(j,i)]= wH.r[j]*exp(  -(wH.r[j] - rho0)*(wH.r[j] - rho0)/sigma/sigma -(wH.z[i] - z0)*(wH.z[i] - z0)/sigma/sigma )*
            exp(I*(v0r*(wH.r[j] - rho0) + v0z*(wH.z[i] - z0)) );
			//if (wH.r[j]>=rho00*0.)
    ...
    wH.normalize();
    cout << "Initial norm: " << wH.norm() << endl << endl;
    ...
    for (int ktime=0; ktime<Ntime; ktime++)
	{
		...
		for(int j=0;j<Nr;j++)
			for(int i=0;i<Nz;i++)
			{
				...
				wH.G[wH.index(j,i)]*=exp(I*faseHankel);
			}
        ...
        cout << "Interpolated norm: " << w.norm() << endl;
        ...
        for(int j=0;j<Nr;j++)
			for(int i=0;i<Nz;i++)
				out1 << ktime << "  " << abs(conj(wH.phi[wH.index(j,i)])*wH.phi[wH.index(j,i)])*wH.r[j] << "  " << abs(conj(w.phi[w.index(j,i)])*w.phi[w.index(j,i)])*w.r[j] << endl;
		
		out2 << ktime << "  " << 1.-wH.norm() << "  " << 1.-w.norm() << endl;
        
		cout << "Propagated (Hankel) error norm: " << 1.-wH.norm() << "\nInterpolated error norm: " << 1.-w.norm() << endl;
