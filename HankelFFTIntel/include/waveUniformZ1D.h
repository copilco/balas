...
public:
	...
	void Numerov1D() 
	{
		...
		if (kmomenta>=0){ //Positive Momentum condition
			for (int i=1; i<Nz-1; i++) {
				...
				complex phim = phi[i-1];
				complex phin = phi[i];	
				
				
				//Doing Numerov's Formula from left to Right
				complex phip = (  2.*( 1.   -  5.*dz*dz*fn/12.   )*phin 
			...
			for (int i=Nz-2; i>0; i--) {
				...
				complex phip = phi[i+1];
				complex phin = phi[i];	
				
				//Doing Numerov's Formula from right to left
				complex phim = (  2.*( 1.  -  5.*dz*dz*fn/12. )*phin 
	...
	*******************************************/
	...
	/********************************************************
	//Complex product of the wavefunction 'phi' by a scalar 'a'
	 ****************************************************/
	...
	/*****************************************************
	//Norm factor by scattering wavefunction solver
	 ***************************************/
	inline complex norm_factor( complex psi, complex Dpsi, double k, double xn)
	{
		return 2.*k*exp( I*k*xn )/( k*psi - I*Dpsi)/sqrt(2.*pi);//2.*k*complex( cos(k*xn) , sin(k*xn) )/( k*psi - I*Dpsi);
	}
	...
	void Continuum_WF( )
	{
		...
		if (kmomenta>=0){//POSITIVE MOMENTUM
			n = Nz-2; //point to derivative of the wave function solution
