...
int main()
{
	...
	fftw_plan pf[Nt];
	for(int i=0;i<Nt;i++)
		pf[i] = fftw_plan_dft_1d(Nt, F2.v+i*Nt, F2.v+i*Nt, FFTW_FORWARD, FFTW_ESTIMATE);
	
	fftw_plan pb[Nt];
	for(int i=0;i<Nt;i++)
		pb[i] = fftw_plan_dft_1d(Nt, F2.v+i*Nt, F2.v+i*Nt, FFTW_BACKWARD, FFTW_ESTIMATE);
	...
	for(int j=0;j<Nr;j++)
	{
		...
		//f.v[j][0]= exp(-(HH.r.v[j]*HH.r.v[j]/w0/w0))*cos(HH.v.v[j]*HH.v.v[j]*HH.r.v[j]*HH.r.v[j]/2.);
		//f.v[j][1]= exp(-(HH.r.v[j]*HH.r.v[j]/w0/w0))*sin(HH.v.v[j]*HH.v.v[j]*HH.r.v[j]*HH.r.v[j]/2.);
	...
	for (int gg=0;gg<Nz;gg++)
	{
		...
		for(int j=0;j<Nr;j++)
			...
			{				
				...
				double psi=(sqrt(K*K - 2*pi*HH.v.v[j][0]*2*pi*HH.v.v[j][0]) - K)*gg*dz+w[i]*w[i]/2.;
	
		//		phi.v[j][0]=(sqrt(K*K - 2*pi*HH.v.v[j][0]*2*pi*HH.v.v[j][0]) - K)*gg*dz+w[i]*w[i]/2.; //The phase
		//		F2.v[j*Nt+i][0]=( aux0r*cos(phi.v[j][0])-aux0i*sin(phi.v[j][0]) );
		//		F2.v[j*Nt+i][1]=( aux0i*cos(phi.v[j][0])+aux0r*sin(phi.v[j][0]) );
				
				F2.v[j*Nt+i][0]=( aux0r*cos(psi)-aux0i*sin(psi) )*wfactor;
				F2.v[j*Nt+i][1]=( aux0i*cos(psi)+aux0r*sin(psi) )*wfactor;
			}
