...
int main()
{
	...
	int Nz = 100;//int(z_max/dz); 
	double dz =0.1;// z_max/(Nz-1);
		...
		*/
		...
		for(int j=0;j<Nr;j++)
			...
			{				
				...
				//complex kinoperator=exp(-I*gg*dz*w[i]*w[i]);
				...
				//F.v[j*Nt+i][0]=( aux0r*real(kinoperator)-aux0i*imag(kinoperator) )/Nt;
				//F.v[j*Nt+i][1]=( aux0i*real(kinoperator)+aux0r*imag(kinoperator) )/Nt;
			}
