...
void laser::Evaluation_Laser_Pulse()
{
	...
	for(int kpulse=0;kpulse<Npulses;kpulse++)
	{
		for(int ktime=0;ktime<Nmaxt;ktime++)
		{
			...
			double arg1=w0[kpulse]*(g.t[ktime]-(clock0[kpulse]+twidth[kpulse]/2.)) + cep0[kpulse];                 // Phase x
			double arg2=w0[kpulse]*(g.t[ktime]-(clock0[kpulse]+twidth[kpulse]/2.)) + cep0[kpulse]+phi_rel[kpulse]; // Phase y
