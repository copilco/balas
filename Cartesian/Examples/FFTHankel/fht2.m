...
if N > 1
   ...
   %figure
    subplot(2,3,1)
   plot( (ifft(a*ko*ro*I.*besselj(n,ko*ro*I))))
   %plot(real(ifft(a*ko*ro*I.*besselj(n,ko*ro*I))),imag(ifft(a*ko*ro*I.*besselj(n,ko*ro*I))));
   title('kernel')
...
if isempty(h)
   ...
   subplot(2,3,3)
   temp2=fft(fft(h.*r,N).*I);
   % plot(temp2)
      plot(fftshift(abs(temp2)))
   title('temp2')
